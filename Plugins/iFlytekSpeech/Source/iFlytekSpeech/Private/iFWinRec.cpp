...
static unsigned int  __stdcall record_thread_proc ( void * para)
{
	...
	//TODO LV 原本这里是GetMessage，但代码编译不过
...
void iFWinRec::destroy_recorder()
{
	if(!rec)
		return;
...
int iFWinRec::open_recorder(unsigned dev, WAVEFORMATEX* fmt)
{
	int ret = 0;
	if(!rec )
		return -RECORD_ERR_INVAL;
...
int iFWinRec::is_record_stopped()
{
	if(rec->state == RECORD_STATE_RECORDING)
		return 0;
...
int iFWinRec::get_default_input_dev()
{
	return WAVE_MAPPER;
}

unsigned iFWinRec::get_input_dev_num()
{
	return waveInGetNumDevs();
}

int iFWinRec::create_recorder(recorder** out_rec,
	void(* on_data_ind)(char* data, unsigned long len, void* user_cb_para), void* user_cb_para)
{
